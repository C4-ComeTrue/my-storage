...
class FileDataHandlerServiceTest {
    ...
    void getFileListCursorIdIsNullTest() {
        given(fileRepository.findAllByParentIdAndUploaderIdOrderByIdDesc(PARENT_ID, USER_ID,
        ...
        then(fileRepository).should(times(1))
    ...
    void getFileListTest() {
        given(fileRepository.findByParentIdAndUploaderIdAndIdLessThanOrderByIdDesc(PARENT_ID, FILE_ID, USER_ID,
            PagingUtil.createPageable(10)))
            .willReturn(List.of(FILE_METADATA));
        ...
        then(fileRepository).should(times(1))
            .findByParentIdAndUploaderIdAndIdLessThanOrderByIdDesc(PARENT_ID, FILE_ID, USER_ID,
                PagingUtil.createPageable(10));
    }
    ...
    void duplicateTest() {
        given(fileRepository.existsByParentIdAndOriginalFileName(PARENT_ID, ORIGINAL_FILE_NAME)).willReturn(false);
        fileDataHandlerService.duplicateBy(PARENT_ID, ORIGINAL_FILE_NAME);
        then(fileRepository).should(times(1)).existsByParentIdAndOriginalFileName(PARENT_ID, ORIGINAL_FILE_NAME);
    }

    @Test
    @DisplayName("특정 폴더 내 파일 중복 테스트 실패")
    void duplicateFailTest() {
        given(fileRepository.existsByParentIdAndOriginalFileName(PARENT_ID, ORIGINAL_FILE_NAME)).willReturn(true);

        assertThrows(ServiceException.class, () -> fileDataHandlerService.duplicateBy(PARENT_ID, ORIGINAL_FILE_NAME));
    }
