...
class FolderDataHandlerServiceTest {
    ...
    void getFolderListCursorIdIsNullTest() {
        given(folderRepository.findAllByParentIdAndUploaderIdOrderByIdDesc(PARENT_ID, USER_ID,
            PagingUtil.createPageable(10)))
            .willReturn(List.of(FOLDER_METADATA));

        List<FolderMetadata> result =
            folderDataHandlerService.getFolderList(PARENT_ID, null, USER_ID, PagingUtil.createPageable(10));
    ...
    void getFolderListTest() {
        ...
        given(folderRepository.findByParentIdAndUploaderIdAndIdLessThanOrderByIdDesc(PARENT_ID, USER_ID, FOLDER_ID,
            PagingUtil.createPageable(10)))
            .willReturn(mockFolderList);

        List<FolderMetadata> result =
            folderDataHandlerService.getFolderList(PARENT_ID, FOLDER_ID, USER_ID, PagingUtil.createPageable(10));

        assertNotNull(result);
        assertEquals(mockFolderList, result);

        then(folderRepository).should(times(1))
            .findByParentIdAndUploaderIdAndIdLessThanOrderByIdDesc(PARENT_ID, USER_ID, FOLDER_ID,
                PagingUtil.createPageable(10));
    }
    ...
    void validateFolderOwnershipTest() {
        ...
        assertThrows(ServiceException.class,
            () -> folderDataHandlerService.validateFolderOwnershipBy(FOLDER_ID, USER_ID));
    }
